...
"""
This is a test for the Vulture Utility Navigation Update - Removing the Facebook button and adding share tool buttons:
...
class UtilityNav(unittest.TestCase):
    ...
    def c_sharetools(self):	
    	    
    	"""
    	Tests the share tools in the nav bar.
    	
    	PASSING CONDITIONS:  Facebook and Twitter Share tools can be clicked on and the appropriate page/popup box appears.
    	FAILING CONDITIONS:  Share tools direct to the wrong page or do not work at all.
    	
    	"""
    	
        driver = self.driver
        test = "Test C - Test for Share Tool Functionality"
        print test
        
        try:
            driver.find_element_by_css_selector(CSS[10].strip('\n')).click()
                
        except AssertionError, e:
            print "FAILURE " + CSS[10]
            self.verificationErrors.append(str(e))
            L.log(BROWSERS[x], test, "FAIL, ELEMENT NOT FOUND", "FACEBOOK LIKE", exception=str(e))
            
        else:
            L.log(BROWSERS[x], test, "PASS, ELEMENT FOUND", "FACEBOOK LIKE")
            
            try:
                driver.switch_to_window('f1-4')
                twittertitle = driver.get_title()
                self.assertEqual("Log In | Facebook", title)
            
            except Exception, e:
            	self.verificationErrors.append(str(e) + "FACEBOOK")
                L.log(BROWSERS[x], test, "FAIL, ELEMENT NOT FOUND", "FACEBOOK LIKE", exception=str(e))
        ...
        try:
            ...
            print "FAILURE " + CSS[9]
            self.verificationErrors.append(str(e))
            L.log(BROWSERS[x], test, "FAIL, ELEMENT NOT FOUND", "TWITTER FOLLOW", exception=str(e))
            
        else:
            L.log(BROWSERS[x], test, "PASS, ELEMENT FOUND", "TWITTER FOLLOW")
            
            try:
                driver.switch_to_window('f1-4')
                twittertitle = driver.get_title()
                self.assertEqual("Vulture (@vulture) on Twitter", twittertitle)
