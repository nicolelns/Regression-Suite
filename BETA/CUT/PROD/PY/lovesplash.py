...
class LoveSplash(unittest.TestCase):
    ...
    def lede_markers(self, u):
	    ...
	    dots = soup.find_all('ol', {'class':'ledeMarkers'})
            ...
            results_writer(str(u) + "articles in lede " + str(len(dots)) + " lede article markers", " Love and War lede markers fail.")
    ...
    def test_text(self):
    	...
    	try:  
    	    self.assertEqual(soup.find('h3').string, "Love & War Newsfeed")
    	    
        except KeyError, e:
            results_writer("", "Love and War Newsfeed Text in header does not match 'Love and War Newsfeed'")
        
        except AssertionError, e:
            results_writer("", "Love and War Newsfeed Text Not on page")
	    ...
	    try:
	        ...
	        for arg in img_dataset:
    	            ...
    	            try:
    	                ...
    	                results_writer(self.shorthead,  'failed.  Check the media queries for the lede on the love splash page.')
    	                pass
    	        ...
    	        for arg in header_dataset:
    	            ...
    	            try:
    	                ...
    	                results_writer(self.shorthead,  'failed.  Check the header for the lede on the love splash page.')
    	                pass
    ...
    def test_prev_next(self):
    	...
    	for arg in dataset:
    	    ...
    	    try:
    	        ...
    	        results_writer(str(arg),  'failed.  Check the prev/next arrows for the lede on the love and war splash page.')
    	        pass
    ...
    def test_metadata(self):
    	...
    	self.assertTrue('Love and War: What Matters to Women, From Body Issues to Politics - TheCut' in soup.find('title').string)
    	
    	dataset = (('meta', {'name':'description', 'content':'Intimate confessions and commentary on topics of interest to women, from body issues to politics to money to relationships.'}), 
    	    ('meta', {'name':'keywords', 'content':'love and war,parenting,sex,relationships,health,love'}),
    	    ('meta', {'name':'content.hierarchy', 'content':'The Cut:Love And War:Index'}),
    	    ('meta', {'name':'content.hierarchy.title', 'content':'The Cut Love and War'}),
    	    ('meta', {'name':'content.type', 'content':'Index'}),
    	    ('meta', {'name':'content.source', 'content':'Online'}),
    	    ('link', {'rel':'canonical', 'href':'http://nymag.com/thecut/love/'})
    	)
    	...
    	for arg in dataset:
    	    ...
    	    try:
    	        ...
    	        results_writer(str(arg),  'failed.  Check the metadata on the love and war splash page.')
    	        pass
    ...
    def test_lpos(self):
    	...
    	try:
            ...
            results_writer("Love and War Splash Page", " Link positioning not on love and war splash page!")
    ...
    def test_entries(self):
	    ...
	    try:
	        ...
	        for arg in dataset:
    	            ...
    	            try:
    	                ...
    	                results_writer(self.shorthead,  'failed.  Check the media queries for entries on the love and war splash page.')
    	                pass
    	...
    	try:
            ...
            results_writer(str(u), " articles on Love and War splash page.  Fail.")
    ...
    def test_bizdev(self):
    	...
    	try:
    	    self.failUnless(re.search('zone: "lovewar"', str(bizdev), re.I))
    	    self.failUnless(re.search('takeover: "loveandwar"', str(bizdev), re.I))
    
        except AssertionError, e:
            results_writer(str(bizdev), "Love and war Splash page has a problem with the bizdev script.")
    ...
    def test_loadmore(self):
	    ...
	    self.assertNotEqual(soup.find('a', {'id':'loadMoreEntries', 'class':'galleryOpen'}), None)
            ...
            results_writer("", "Load More button is not on love and warsplash page")
...
if __name__ =='__main__':
    ...
    for key in envs.keys():
        ...
        results_writer(str(datetime.datetime.now()), " BEGINNING LOVE AND WAR SPLASH PAGE TEST")
        results_writer(data['url'], " ")
        soup = page_parser()    
        suite = unittest.TestLoader().loadTestsFromTestCase(LoveSplash)
        unittest.TextTestRunner(verbosity=2).run(suite)
        #results = open(path[:-4] + '.html', 'wb')
        #runner = HTMLTestRunner.HTMLTestRunner(stream=results, title="Love and War Splash Page", description='Test Results for Love and War Splash Page')
        #runner.run(suite)
