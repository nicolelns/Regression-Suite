...
PAGES = ('beauty/', 'news/', 'shopping/', 'love/',
        'street-style/', 'runway/', 'celebrities/', 'fashion/')


"""
This test checks for features common to all splash 
pages, such as css sheets, facebook login,
the footer, global navigation, cut navigation, 
The Cut logo, etc.  Specific elements for splash pages are 
in the corresponding splash test: i.e., beautysplash.py
...
class Splash(unittest.TestCase):
    ...
    def test_facebook(self):
    	...
    	self.assertNotEqual(self.soup.find('meta', attrs={'property':'og:url', 'content':'http://nymag.com/thecut/' + filename[-2] + '/'}), None)
    ...
    def test_ads(self):
    	...
    	self.assertNotEqual(self.soup.find('div', attrs={'class':'nym-ad', 'id':'leaderboard-tablet-mobile', 'data-ad-size':'468x60,480x60,480x61'}), None)
    	self.assertNotEqual(self.soup.find('div', attrs={'class':'nym-ad', 'id':'leaderboard-mobile', 'data-ad-size':'300x50,320x50'}), None)
    	
    	if not re.match(filename[-2], 'shopping'):
    	    self.assertNotEqual(self.soup.find('div', attrs={'class':'nym-ad', 'id':'secondary-flex', 'data-ad-size':'300x250,300x600,300x1050'}), None)
    	
        else:
            pass
    
	#self.assertEqual(self.soup.find('div', attrs={'class':'nym-ad', 'id':'secondary-scroll', 'data-ad-size':'300x250,300x251'}), None)
    	
    	self.assertNotEqual(self.soup.find('div', attrs={'class':'nym-ad', 'id':'footer-tablet-desktop', 'data-ad-size':'728x90,728x91'}), None)
    	self.assertNotEqual(self.soup.find('div', attrs={'class':'nym-ad', 'id':'footer-tablet-mobile', 'data-ad-size':'468x60,480x60,480x61'}), None)
    ...
    #@unittest.skipIf(page == 'shopping/', "Goods page does not have partners module")
    def test_partners(self):
    	
        if not re.match(str(filename[-2]), 'shopping'): 
    	    self.assertNotEqual(self.soup.find('div', attrs={'id':'nym-partner-module'}), None)
...
for x in range(0,3):
    ...
    for page in PAGES:
        ...
        results = open('../../DATA/HTML/SPLASH/' + filename[-2] + '.' + ENV + '.basicsplash.html', 'wb')
        print "TESTING " + baseurl
        suite = unittest.TestLoader().loadTestsFromTestCase(Splash)
        unittest.TextTestRunner(verbosity=2).run(suite)
        runner = HTMLTestRunner.HTMLTestRunner(stream=results, title=filename[-2], description='General Results for ' + baseurl + ' Splash Page on ' + ENV)
        runner.run(suite)
        results.close()
