...
TEST = "Star Market Module - Desktop - Vulture Home Page"
...
CSS = open('../data/text/starmarket.css.txt', 'r').readlines()		# a = presence; Is it there?
DATA = pickle.load(open('../data/pickle/starmarket.data.p', 'rb')) 	# c = function; Does the module work?
...
class StarMarket(unittest.TestCase):
	...
	for each in keys:
	    ...
	    silo = values[m]
            ...
            if re.search(text, silo[1], re.I):
            	L.log(BROWSERS[x], TEST, "PASS, Each article has permalink", d)
        ...
        if sel.is_text_present('The real value of Hollywood actors'):
            L.log(BROWSERS[x], TEST, "PASS, Correct text is present", "The real value of Hollywood actors")
            
        else:
            L.log(BROWSERS[x], TEST, "FAIL, Correct text is NOT present", "The real value of Hollywood actors")
