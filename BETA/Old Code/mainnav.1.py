...
CSS = open('../data/text/mainnav.css.txt', 'r').readlines()		# a = presence; Is it there?
DATA = pickle.load(open('../data/pickle/mainnav.data.p', 'rb')) 	# c = function; Does the module work?
...
class MainNav(unittest.TestCase):
	...
	for each in DATA:
        ...
        # Third, make sure the correct number of links are in the Main Navigation bar    
            
        count = sel.get_css_count("css=" + CSS[1])
        
        if count != 4:
            L.log(BROWSERS[x], TEST, "FAIL, Missing Navigation Links", str(count) + " links found")
