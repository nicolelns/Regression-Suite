...
CSS = open('../data/vulture.home.partners.css.txt', 'r').readlines()		# a = presence; Is it there?
CONTENT = open('../data/vulture.home.partners.content.txt', 'r').readlines()
DATA = open('../data/vulture.home.partners.data.txt', 'r').readlines()  	# c = function; Does the module work?
...
class Partners(unittest.TestCase):
    ...
    # This test has yet to accomodate the fact that only 4 of the 5 feeds will display at any one time
	...
	sel = self.selenium
        ...
        huff = sel.get_css_count("css=" + CSS[10])
        buzz = sel.get_css_count("css=" + CSS[13])
        fone = sel.get_css_count("css=" + CSS[13])
        
        if (wonder != 5) or (holly != 5) or (huff != 5) or (buzz != 5) or (fone != 5):
