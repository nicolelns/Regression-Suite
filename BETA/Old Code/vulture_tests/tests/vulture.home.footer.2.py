...
TEST = "Vulture Footer Section"

L = Logger.MainLogger(BASEURL, TEST)

CSS = open('../data/vulture.home.footer.css.txt', 'r').readlines()		# a = presence; Is it there?
CONTENT = "Also In Our Network:"
DATA = open('../data/vulture.home.footer.data.txt', 'r').readlines()  	        # c = function; Does the module work?
...
Test 'b' is a 'content' test: contain the relevant data?  
...
class Footer(unittest.TestCase):
	...
	for each in DATA:
	    ...
	    d = DATA[n].strip('\n') 
            ...
            try:
                ...
                if re.search(text, title, re.I):
                    ...
                    print "Title: " + title, ", link text: " + text
        ...
        # Third, make sure the correct number of links are in the Footer    
        ...
        # Fourth, make sure the From Our Network statement is on the page
            	
        if sel.is_text_present(CONTENT):
            L.log(BROWSERS[x], TEST, "PASS, text present", CONTENT)
            print "Network statement is present"
