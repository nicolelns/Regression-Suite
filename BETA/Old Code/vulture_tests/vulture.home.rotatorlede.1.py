...
S.rotatorlede()			# Call the relevant BeautifulSoup function (usually same name as test)

CSS = open('../data/vulture.home.rotatorlede.css.txt', 'r').readlines()		# a = presence; Is it there?
CONTENT = open('../data/vulture.home.rotatorlede.content.txt', 'r').readlines() # b = content; Does it have the right stuff? 
DATA = open('../data/vulture.home.rotatorlede.data.txt', 'r').readlines()  	# c = function; Does the module work?
      
x = 0

"""This is a regression test for the Rotating Lede module on Vulture's home page"""
...
class RotatorLede(unittest.TestCase):
	...
	for each in DATA:
        ...
        if S.rotatorlede() != (5,5):
            print "Missing Content in Rotator Lede!"
            print S.rotatorlede()
...
for each in BROWSERS:
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(RotatorLede)
