...
TEST = "New Reviews Module - Desktop - Vulture Home Page"

L = Logger.MainLogger(BASEURL, TEST)
S = vultureSoup.Parser()

S.newreviews()			# Call the relevant BeautifulSoup function (usually same name as test)

CSS = open('../data/text/newreviews.css.txt', 'r').readlines()		# a = presence; Is it there?
DATA = pickle.load(open('../data/pickle/newreviews.data.p', 'rb')) 	        # c = function; Does the module work?
      
x = 0

"""This is a regression test for the New Reviews module on Vulture's home page"""
...
class NewReviews(unittest.TestCase):
    ...
    # Makes sure the page loads
	    ...
	    if data is not None and data != "tab" and data != "permalink":        
		    ...
		    print "FAILURE " + data, " does not load"
		    L.log(BROWSERS[x], TEST, "FAIL, IMAGE DOES NOT LOAD ", str(e) + data)
		
	        else:
	    	    L.log(BROWSERS[x], TEST, "PASS, Image loads", data)
		    sel.go_back()
	            sel.wait_for_page_to_load("50000")
